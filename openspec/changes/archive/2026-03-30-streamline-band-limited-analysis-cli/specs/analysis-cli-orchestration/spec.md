...
The system SHALL export figures and table outputs from the staged workflow into report directories without requiring users to manually transform cached results.

#### Scenario: Reports are rendered after activity and connectivity stages complete
- **WHEN** the report stage runs after staged analysis caches exist
- **THEN** the system SHALL write figure outputs and Excel table exports for the available activity and connectivity results

#### Scenario: Only a subset of staged outputs exists
- **WHEN** the report stage runs and some staged result caches are absent
- **THEN** the report stage SHALL export reports only for the result sets that are available
