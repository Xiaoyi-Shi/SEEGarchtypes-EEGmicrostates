...
The orchestration layer SHALL allow exploratory coupling analyses to reuse the existing staged index, EEG microstate labels, staged EEG GFP artifacts, and the branch-specific SEEG exploratory inputs required by each method, including staged SEEG region signal caches for region- and network-based methods and subject-level SEEG field-state artifacts for electrode-space field-state methods, without recomputing those reusable upstream artifacts.
...
- **AND** the exploratory coupling entry point SHALL expose only maintained region-signal exploratory analyses plus the supported direct EEG-SEEG state-coupling analyses, the supported GFP-informed global-coupling analyses, and the supported SEEG global-field-state analyses
