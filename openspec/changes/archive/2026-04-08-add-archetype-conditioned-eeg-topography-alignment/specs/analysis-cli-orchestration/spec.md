...
The system SHALL expose a public command surface centered on the focused staged `1-40 Hz` analysis workflow, SHALL keep `IDE_A` as the default analysis state while allowing `IDE_S` as an optional override, SHALL expose the public SEEG staging step as `run-seeg-regions`, and SHALL allow only the maintained exploratory analyses explicitly supported by the region-based workflow plus the opt-in direct EEG-SEEG state-coupling branch, GFP-informed global-coupling branch, and SEEG global-field-state exploratory branches, including group archetype summaries, native `4 ms` fine-lag field-state follow-ups, SEEG-led switching analyses, and archetype-conditioned EEG topography alignment.
...
- **WHEN** group SEEG field-state archetype caches, fine `4 ms` field-state lag-coupling caches, SEEG-led switching caches, or archetype-conditioned EEG topography caches exist for the selected analysis state
- **THEN** the report stage SHALL export the corresponding archetype summaries, fine-lag curves, SEEG-to-EEG transition outputs, conditioned EEG scalp-map panels, similarity matrices, and archetype-conditioned EEG preference figures and tables
